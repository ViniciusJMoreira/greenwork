...
/**
 * Andamento ore per giorno con serie aggiuntive filtrate per tipo lavoro.
 * @param {Array} registri
 * @param {Array<{ key: string, nomi: string[] }>} gruppi
 * @param {number} limit
 * @returns {Array<{ data: string, ore: number, [key]: number }>}
 */
export function getAndamentoMulti(registri, gruppi, limit = 14) {
  const giorniSet = new Set(registri.map((r) => r.data))
  return Array.from(giorniSet)
    .sort()
    .slice(-limit)
    .map((d) => {
      const dayRec = registri.filter((r) => r.data === d)
      const toOre = (arr) =>
        parseFloat(fmtOreDecimale(arr.reduce((acc, r) => acc + calcMin(r.inizio, r.fine), 0)).replace(',', '.')) || 0
      const entry = { data: d.slice(5), ore: toOre(dayRec) }
      gruppi.forEach(({ key, nomi }) => {
        const set = new Set(nomi)
        entry[key] = toOre(dayRec.filter((r) => r.lavoro && set.has(r.lavoro)))
      })
      return entry
    })
}
