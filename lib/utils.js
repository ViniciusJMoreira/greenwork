 ...
 * Converte minuti decimali in formato decimale (es. 90 → 1.5)
 */
...
/**
 * Restituisce { meseMin, meseMax } del mese corrente in formato "YYYY-MM-DD"
 * per limitare i date input al mese corrente.
 */
export function getMeseLimiti() {
  const d = new Date();
  const pad = (n) => String(n).padStart(2, "0");
  const y = d.getFullYear();
  const m = pad(d.getMonth() + 1);
  const ultimo = new Date(y, d.getMonth() + 1, 0);
  return {
    dataOggi: `${y}-${m}-${pad(d.getDate())}`,
    meseMin:  `${y}-${m}-01`,
    meseMax:  `${ultimo.getFullYear()}-${pad(ultimo.getMonth() + 1)}-${pad(ultimo.getDate())}`,
  };
}
