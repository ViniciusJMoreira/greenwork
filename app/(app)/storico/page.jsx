...
function formatHeader(dataStr) {
  ...
  return `${dn} ${d} ${fmtData(dataStr)}`;
}

function Pill({ children, color = "var(--primary)", bg = "var(--primary-faint)" }) {
  return (
    <span
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1 flex-wrap">
          ...
          {record.note && (
            <span className="text-xs italic truncate" style={{ color: "var(--text-faint)" }}>
              {record.note}
            </span>
          )}
        </div>
      ...
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.color = "var(--destructive)"; e.currentTarget.style.background = "var(--primary-faint)"; }}
...
export default function StoricoPage() {
  ...
  const hasFilters = startDate || endDate || selectedCantiere !== "all" || selectedLavoro !== "all";
  const activeCount = [startDate, endDate, selectedCantiere !== "all", selectedLavoro !== "all"].filter(Boolean).length;

  async function handleDelete(id) {
    if (!window.confirm("Eliminare questo turno?\nVerrà sincronizzato con Google Sheets.")) return;
  ...
  function reset() {
    ...
    setSelectedCantiere("all"); setSelectedLavoro("all");
  }

  const filtered = useMemo(() => turni.filter((t) => {
    if (startDate && t.data < startDate) return false;
    if (endDate && t.data > endDate) return false;
    if (selectedCantiere !== "all" && String(t.cantiere_id) !== selectedCantiere) return false;
    if (selectedLavoro !== "all" && String(t.lavoro_id) !== selectedLavoro) return false;
    return true;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold" style={{ color: "var(--text)" }}>Storico Turni</h1>
        ...
        {open && (
          <div className="border-t px-4 pb-4 pt-3" style={{ borderColor: "var(--border)" }}>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <FilterInput label="Dal" value={startDate} onChange={setStartDate} />
              <FilterInput label="Al" value={endDate} onChange={setEndDate} />
              <FilterSelect label="Cantiere" value={selectedCantiere} onChange={setSelectedCantiere}>
                <option value="all">Tutti i cantieri</option>
                {(cantieri ?? []).map((c) => (
                  <option key={c.id} value={String(c.id)}>{c.cantiere}</option>
                ))}
              </FilterSelect>
              <FilterSelect label="Tipo Lavoro" value={selectedLavoro} onChange={setSelectedLavoro}>
      ...
      {gruppi.length === 0 ? (
        ...
        >
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>
            {hasFilters ? "Prova a modificare i filtri" : "Inserisci il tuo primo turno"}
          </p>
        </div>
      ) : (
