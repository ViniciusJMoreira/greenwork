...
// Variante comune per entrance con prospettiva 3D
const cardVariant = (i = 0) => ({
  initial: { opacity: 0, y: 18, rotateX: 8, scale: 0.97 },
  animate: { opacity: 1, y: 0, rotateX: 0, scale: 1 },
  transition: { type: "spring", damping: 22, stiffness: 300, delay: i * 0.07 },
});

function Section({ title, icon: Icon, children, index = 0 }) {
  return (
    <motion.div
      {...cardVariant(index)}
...
function KpiCard({ icon: Icon, value, label, trend, index = 0 }) {
  const up = trend > 0;
...
export default function DashboardPage() {
  ...
  const turniMese = useMemo(() => turni.filter((t) => t.data?.startsWith(meseStr)), [turni, meseStr]);
  const turniPrev = useMemo(() => turni.filter((t) => t.data?.startsWith(prevMeseStr)), [turni, prevMeseStr]);
  ...
  const media = stats.giorniLavorati ? (stats.minutiTotali / 60 / stats.giorniLavorati).toFixed(1) : "0";
  ...
  const lavoriData = useMemo(() => {
    const map = {};
    turniMese.forEach((t) => { if (t.lavoro) map[t.lavoro] = (map[t.lavoro] || 0) + calcMin(t.inizio, t.fine); });
    const total = Object.values(map).reduce((a, b) => a + b, 0);
    return Object.entries(map)
      .map(([lavoro, min]) => ({ lavoro, min, pct: total ? Math.round((min / total) * 100) : 0 }))
  ...
  const macchinariData = useMemo(() => {
    const map = {};
    turniMese.forEach((t) => { if (t.macchinario) map[t.macchinario] = (map[t.macchinario] || 0) + calcMin(t.inizio, t.fine); });
    return Object.entries(map)
      .map(([mezzo, min]) => ({ mezzo, ore: parseFloat((min / 60).toFixed(1)) }))
  ...
  const empty = <p className="text-center text-sm py-6" style={{ color: "var(--text-faint)" }}>Nessun dato disponibile</p>;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard icon={Clock} value={`${oreTot}h`} label="Ore Totali" trend={trend(stats.minutiTotali, statsPrev.minutiTotali)} index={0} />
        <KpiCard icon={CalendarDays} value={stats.giorniLavorati} label="Giorni Lavorati" trend={trend(stats.giorniLavorati, statsPrev.giorniLavorati)} index={1} />
        <KpiCard icon={TrendingUp} value={`${media}h`} label="Media Giornaliera" trend={0} index={2} />
        <KpiCard icon={Building2} value={stats.numCantieri} label="Cantieri Attivi" trend={trend(stats.numCantieri, statsPrev.numCantieri)} index={3} />
      </div>

      {/* Andamento ore */}
      <Section title={`Andamento Ore — ${MESI[today.getMonth()]}`} index={0}>
        {orePerGiorno.length === 0 ? empty : (
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={orePerGiorno} margin={{ top: 8, right: 4, left: -16, bottom: 0 }}>
              ...
              <Area
                type="monotone" dataKey="ore" name="Ore"
                stroke="var(--primary)" strokeWidth={2} fill="url(#oreGrad)"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Section title="Distribuzione Cantieri" icon={HardHat} index={1}>
          {pieData.length === 0 ? empty : (
            <>
              <ResponsiveContainer width="100%" height={150}>
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={42} outerRadius={68} dataKey="ore" paddingAngle={3}>
        ...
        <Section title="Ore per Tipo Lavoro" icon={Wrench} index={2}>
          {lavoriData.length === 0 ? empty : (
            <div className="flex flex-col gap-3">
              {lavoriData.map((l, i) => (
                <div key={l.lavoro}>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="truncate font-medium" style={{ color: "var(--text)" }}>{l.lavoro}</span>
                    <span className="shrink-0 ml-2" style={{ color: "var(--text-muted)" }}>{fmtOre(l.min)} · {l.pct}%</span>
                  </div>
                  <AnimBar pct={l.pct} delay={0.3 + i * 0.09} />
                </div>
              ))}
            </div>
          )}
      ...
      {/* Macchinari + Heatmap */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Section title="Utilizzo Macchinari" index={3}>
          {macchinariData.length === 0 ? (
            <p className="text-center text-sm py-6" style={{ color: "var(--text-faint)" }}>Nessun macchinario registrato</p>
          ) : (
            <div className="flex flex-col gap-3">
              {macchinariData.map((m, i) => {
                const pct = (m.ore / macchinariData[0].ore) * 100;
                return (
                  <div key={m.mezzo}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="truncate font-medium" style={{ color: "var(--text)" }}>{m.mezzo}</span>
                      <span className="font-bold shrink-0 ml-2" style={{ color: "var(--primary)" }}>{m.ore}h</span>
                    </div>
                    <AnimBar pct={pct} delay={0.3 + i * 0.09} />
                  </div>
                );
              })}
            </div>
          ...
          <div className="px-5 py-4">
            <div className="grid grid-cols-7 gap-1 mb-1">
              {["L", "M", "M", "G", "V", "S", "D"].map((d, i) => (
                <div key={i} className="text-center text-[10px] font-bold" style={{ color: "var(--text-faint)" }}>{d}</div>
              ))}
            </div>
